...
import { buildHunter, poseHunter } from "./hunter";
...
export class CrimsonGame {
	...
	constructor(canvas) {
		...
		this.camera = new THREE.PerspectiveCamera(78, 1, .08, 340);
	...
	async loadWorld() {
		...
		this.mats.column = new THREE.MeshStandardMaterial({
			map: colT,
			color: 0xc4b0aa,
			roughness: 0.62,
			metalness: 0.28,
		...
		this.mats.ember = new THREE.MeshStandardMaterial({
			color: C.ember,
			emissive: colourShift(C.ember, this.profile.code, 0.04),
			emissiveIntensity: 1.15 + this.profile.glow * .35,
			roughness: .35,
			metalness: .2
		});
		this.mats.body = new THREE.MeshStandardMaterial({
			color: C.void,
			roughness: .55,
			metalness: .25,
			emissive: C.crimson,
		...
		const card = (url, fog = true) => {
			...
			return new THREE.MeshBasicMaterial({ map: t, fog, side: THREE.DoubleSide, toneMapped: false });
		};
		...
		this.scene.fog = new THREE.Fog(C.void, 90, 280);
		const hemi = new THREE.HemisphereLight(C.crimson, C.night, 0.55);
		const dir = new THREE.DirectionalLight(mixHex(C.gold, C.arterial, 0.45), 0.72);
		dir.position.set(-20, 80, 40);
		const fill = new THREE.PointLight(C.arterial, 2.6, 90, 1.6);
		fill.position.set(0, 18, 20);
		hemi.layers.enable(0);
		hemi.layers.enable(WEAPON_LAYER);
		dir.layers.enable(0);
		dir.layers.enable(WEAPON_LAYER);
		this.scene.add(hemi, dir, fill);
		const sky = new THREE.Mesh(new THREE.SphereGeometry(380, 24, 16), new THREE.MeshBasicMaterial({
			map: skyT,
			side: THREE.BackSide,
			depthWrite: false,
			fog: false,
	...
	enemyMesh(kind) {
		...
		const w = h * (kind === "boss" ? 0.68 : 0.74);
		if (art) {
			const plane = new THREE.Mesh(new THREE.PlaneGeometry(w, h), art);
			plane.position.y = h * 0.48;
			plane.rotation.y = Math.PI;
			plane.userData.billboard = true;
			g.add(plane);
		} else {
			g.add(mesh(new THREE.CapsuleGeometry(.28, 1, 4, 8), this.mats.body, 0, .9, 0));
		}
		const disc = new THREE.Mesh(
			new THREE.CircleGeometry(w * 0.22, 12),
		...
		const ember = new THREE.Mesh(
			new THREE.SphereGeometry(kind === "boss" ? 0.22 : 0.09, 8, 8),
			this.mats.ember,
		);
		ember.position.y = h * 0.82;
		g.add(ember);
		return g;
	}
	spawnBoss() {
	...
	applyWorldMoments(dt) {
		...
		if (this.scene.fog && this.scene.fog.isFog) {
			...
			this.scene.fog.near = 90 / n;
			this.scene.fog.far = 280 / Math.sqrt(n);
	...
	setQuality(q) {
		...
		const pr = this.quality >= 1080 ? Math.min(2, dpr) : this.quality >= 720 ? Math.min(1.25, dpr) : 0.62;
		this.renderer.setPixelRatio(pr);
		this.renderer.toneMapping = this.quality >= 720 ? THREE.ACESFilmicToneMapping : THREE.NoToneMapping;
		this.renderer.toneMappingExposure = this.quality >= 1080 ? 1.18 : 1.05;
	...
	look(dt) {
		if (this.overlayOpen() || (this.grace > 0.25)) {
	...
	spawnProj(fromEnemy, speed, dmg, radius, origin, dir, grav = false) {
		...
		if (!p) {
			const m = new THREE.Mesh(new THREE.SphereGeometry(.12, 8, 8), this.mats.ember);
			p = {
				mesh: m,
				vx: 0,
				vy: 0,
	...
	exposeControls() {
		...
		const probe = {
			...
			setPos: (x, z) => {
				this.px = x;
				this.pz = z;
				this.py = this.floorBelow(x, z, 14);
				this.vx = 0;
				this.vz = 0;
				this.vy = 0;
				this.grounded = true;
