...
export function GameApp() {
  ...
  useEffect(() => {
    ...
    void preloadArt(setLoadP).then(() => setBooted(true));
  }, []);

  if (!booted) {
    return (
      <div className="relative grid h-dvh w-full place-items-center overflow-hidden bg-bg font-sans text-fg">
        <div className="w-64 text-center">
          <p className="font-display text-sm tracking-[0.3em] text-muted">LOADING THE PALACE</p>
          <div className="mt-4 h-1 overflow-hidden rounded-full bg-raised">
            <div className="h-full bg-crimson" style={{ width: `${Math.round(loadP * 100)}%` }} />
          </div>
          <p className="mt-2 font-mono text-xs tracking-widest text-subtle">{Math.round(loadP * 100)}%</p>
        </div>
      </div>
    );
  }
    ...
    <>
      ...
      {!ready ? (
        <div className="absolute inset-0 z-20 grid place-items-center bg-bg">
          <p className="font-display text-sm tracking-[0.3em] text-muted">OPENING THE RIFT</p>
        </div>
      ) : null}
      <Hud />
      <WeaponBag />
  ...
  return (
    <div className="pointer-events-none absolute inset-0 z-10">
      ...
      <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
        {hud.scoped ? null : (
          <>
            <div className="size-3 rounded-full border-[1.5px] border-crimson shadow-[0_0_12px_#c41e3a]" />
            <div
...
function MobileControls() {
  ...
  return (
    <div className="pointer-events-none absolute inset-0 z-20">
      <div className="absolute bottom-28 left-5 size-28 rounded-full border border-border/80 bg-surface/25" />
      <div className="absolute bottom-6 right-2 flex flex-col items-end gap-1.5 pointer-events-auto pb-[max(0.5rem,env(safe-area-inset-bottom))]">
        ...
        <div className="flex gap-2">
          <TapBtn label="TREE" onTap={() => window.__crimsonInput?.tree?.()} />
