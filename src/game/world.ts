...
export class World {
  ...
  private ground() {
    const geo = new THREE.PlaneGeometry(280, 280, 24, 24);
  ...
  private thresholdRift() {
    ...
    const ring = new THREE.Mesh(
      new THREE.TorusGeometry(6.5, 0.22, 8, 32),
      this.mats.energy,
    );
  ...
  private runeCircle(x: number, z: number, r: number) {
    const ring = new THREE.Mesh(
      new THREE.TorusGeometry(r, 0.08, 8, 32),
      this.mats.energy,
    );
  ...
  private energyRiver(x1: number, z1: number, x2: number, z2: number, w: number) {
    ...
    const mesh = new THREE.Mesh(new THREE.PlaneGeometry(w, len), this.mats.energy);
  ...
  private palace() {
    ...
    for (const x of [-10, 0, 10]) {
      this.cyl(C, x, 0, -4, 0.7, 0.85, 12, 8);
    }
    ...
    const innerLight = new THREE.PointLight(0xc41e3a, 5.5, 42, 2);
  ...
  private floatRunes() {
    ...
    for (let i = 0; i < n; i++) {
      ...
      const s = 0.5 + this.rng() * this.profile.runeScale;
  ...
  private rynaraArchive() {
    for (let i = 0; i < 10; i++) {
      ...
      const h = 7 + (i % 4) * 2.4;
      this.box(this.mats.column, x, 0, z, 2.0, h, 2.0);
    ...
    const l = new THREE.PointLight(0xc41e3a, 3.4, 24, 2);
  ...
  private nyxaraRise() {
    ...
    const bridges: [number, number, number, number, number, number][] = [
      [-40, 1.7, -18, -48, 3.9, -30],
      [-48, 3.9, -30, -58, 6.3, -40],
      [-58, 6.3, -40, -52, 8.9, -52],
      [-52, 8.9, -52, -42, 7.1, -44],
    ];
    for (const [x1, y1, z1, x2, y2, z2] of bridges) {
      const dx = x2 - x1;
      const dz = z2 - z1;
      const len = Math.hypot(dx, dz);
    ...
    this.billboard("portraitNyxara", -48, 11.2, -50, 3.0, 5.2, 0);
  ...
  private pickupsAndSpawns() {
    ...
    const ammo: [number, number, number][] = [
      [10, 0.5, 70], [-12, 0.5, 64], [40, 0.5, 10], [-44, 0.5, -8], [6, 0.5, -50], [24, 0.5, -30],
      [-48, 4.0, -30], [64, 0.5, -18], [12, 0.5, -90], [-16, 0.5, -90], [90, 0.5, -16],
      [8, 1.4, 52], [-8, 1.4, 42], [-70, 0.5, 8],
    ];
    for (const [x, y, z] of ammo) this.addCrate("ammo", x, y, z);
    const hp: [number, number, number][] = [
      [0, 0.5, 62], [-30, 0.5, 20], [28, 0.5, -12], [0, 1.6, -12], [-52, 9.0, -52], [80, 1.1, 4],
      [0, 0.5, -82], [-78, 0.5, 4],
    ];
    for (const [x, y, z] of hp) this.addCrate("health", x, y, z);
  ...
  private livingPillar(x: number, z: number, y: number, name: NameId) {
    ...
    const body = new THREE.Mesh(
      new THREE.BoxGeometry(1.85, h, 1.85),
      this.mats.body,
    );
    ...
    const face = new THREE.Mesh(
      new THREE.PlaneGeometry(1.45, h * 0.78),
      faceMat,
    );
  ...
  syncMoment(opts: { ember: boolean; jump: number; night: number; tide: number; resist: number }) {
    ...
    const e = this.mats.energy;
    if (e instanceof THREE.MeshBasicMaterial) {
      e.opacity = opts.night > 1.8 ? 0.45 : 0.92;
    }
  }
  ...
  setGateOpen(open: boolean) {
    ...
    if (this.gateCollider) {
      if (open) {
        this.gateCollider.maxY = -1;
      } else {
        this.gateCollider.maxY = 11;
      }
    }
  }

  getZone(x: number, y: number, z: number): { id: string; name: string } {
  ...
  update(t: number) {
    ...
    const e = this.mats.energy;
    if (e instanceof THREE.MeshBasicMaterial && e.map) {
      e.map.offset.y = (t * 0.12 * this.tide) % 1;
    }
    if (this.emberField) {
    ...
    if (this.floatInst) {
      this.floatInst.rotation.y = t * 0.04 * this.skySpin;
    }
    if (this.skyPts) {
    ...
    if (this.galaxyPts) {
      this.galaxyPts.rotation.y = t * 0.008 * this.skySpin;
    }
    const dt = 1 / 60;
    for (const s of this.swarms) {
