import { C as emptyActions, S as anyCode, _ as INTERACT_GLYPH, a as GATE_MOMENT, b as STICK_SPRINT, c as CHARACTERS, d as buildProfile, f as mulberry32, g as DEADZONE, h as ARM_FROM_DPAD, i as useGame, l as VIEW, m as streamSeed, n as modsFrom, o as NAMES, p as padCode, r as skillById, s as nameById, u as WEAPONS, v as KEY, w as radialDeadzone, y as PAD } from "./routes-Drwd2sjo.mjs";
import { A as Raycaster, C as OctahedronGeometry, D as Points, E as PointLight, F as TextureLoader, I as Timer, L as TorusGeometry, M as SRGBColorSpace, N as Scene, O as PointsMaterial, P as SphereGeometry, R as Vector2, S as Object3D, T as PlaneGeometry, _ as InstancedMesh, a as CapsuleGeometry, b as MeshBasicMaterial, c as Color, d as DirectionalLight, f as DynamicDrawUsage, g as IcosahedronGeometry, h as HemisphereLight, i as BufferGeometry, j as RepeatWrapping, k as Quaternion, l as ConeGeometry, m as Group, n as BoxGeometry, o as CircleGeometry, p as Fog, r as BufferAttribute, s as ClampToEdgeWrapping, t as WebGLRenderer, u as CylinderGeometry, v as MathUtils, w as PerspectiveCamera, x as MeshStandardMaterial, y as Mesh, z as Vector3 } from "../_libs/three.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/engine-D3-Kak67.js
/**
...
var Input = class {
	...
	poll() {
		...
		mx += this.touchMoveX;
		my += this.touchMoveY;
		const stickSprint = Math.hypot(this.touchMoveX, this.touchMoveY) >= STICK_SPRINT;
	...
	hasActivity() {
		return this.fireHeld || this.touchLookActive || this.keys.size > 0 || Math.abs(this.moveX) + Math.abs(this.moveY) > .04 || Math.abs(this.lookStickX) + Math.abs(this.lookStickY) > .04 || this.jumpPressed || this.interactPressed;
	}
	endFrame() {
...
var World = class {
	...
	ground() {
		const geo = new PlaneGeometry(280, 280, 24, 24);
	...
	thresholdRift() {
		...
		this.group.add(veil);
		const lamp = new PointLight(16724821, 7, 28, 2);
	...
	grandStair() {
		...
		const lamp = new PointLight(16733491, 8, 36, 2);
		lamp.position.set(0, 8, 50);
		this.group.add(lamp);
		const lamp2 = new PointLight(16737860, 5, 28, 2);
	...
	horizonArt() {
		...
		if (skyline) {
			const m = new Mesh(new PlaneGeometry(90, 42), skyline);
			m.position.set(0, 18, -48);
			m.rotation.y = 0;
			this.group.add(m);
		}
		if (this.mats.cardStairs) {
			const m = new Mesh(new PlaneGeometry(36, 22), this.mats.cardStairs);
			m.position.set(0, 14, -38);
			m.rotation.y = 0;
			this.group.add(m);
		}
	...
	palace() {
		...
		for (const x of [
			...
			0,
			10
		]) this.cyl(C, x, 0, -4, .7, .85, 12, 8);
		this.box(C, 0, 0, -16.5, 7, 1.1, 4.2);
		this.box(C, 0, 1.1, -17.6, 4.4, 3.4, 1.2);
		this.runeCircle(0, -14, 4.5);
		this.billboard("cardThrone", 0, 8, -19.2, 12, 10, 0);
		this.billboard("portraitOrigin", -8, 6.2, -18.4, 4.2, 6.2, 0);
		const innerLight = new PointLight(12852794, 5.5, 42, 2);
		innerLight.position.set(0, 7, -8);
		this.group.add(innerLight);
		const gateLight = new PointLight(16720452, 7, 48, 2);
	...
	silhouette() {
		...
		const glow = new PointLight(16711731, 3.8 + this.profile.glow, 90, 2);
	...
	floatRunes() {
		...
		for (let i = 0; i < n; i++) {
			...
			const s = .5 + this.rng() * this.profile.runeScale;
	...
	vaelithCourt() {
		...
		for (const [x, z, h] of [
			...
			const flame = new PointLight(16729122, 2.2, 16, 2);
			flame.position.set(x, h * .7, z);
			this.group.add(flame);
		}
		this.runeCircle(0, -82, 6);
		const flame = new PointLight(16733491, 6, 28, 2);
	...
	rynaraArchive() {
		...
		const l = new PointLight(12852794, 3.4, 24, 2);
	...
	addRune(name, x, y, z) {
		...
		this.group.add(g);
		const light = new PointLight(16724821, 1.6 + this.profile.glow, 10, 2);
		g.add(light);
		this.bobbers.push({
			o: g,
			y,
	...
	livingPillar(x, z, y, name) {
		...
		this.group.add(face);
		const light = new PointLight(16711731, 1.4 + this.profile.glow * .6, 14, 2);
	...
	milkyWay() {
		...
		for (let i = 0; i < n; i++) {
			...
			const hot = this.rng();
			colors[i * 3] = 1;
			colors[i * 3 + 1] = .12 + hot * .35;
			colors[i * 3 + 2] = .28 + hot * .45;
		}
		const geo = new BufferGeometry();
		geo.setAttribute("position", new BufferAttribute(positions, 3));
	...
	setGateOpen(open) {
		...
		if (this.gateCollider) {
			if (open) this.gateCollider.maxY = -1;
			else this.gateCollider.maxY = 11;
		}
	}
	getZone(x, y, z) {
...
function buildHunter(face, dress, ember) {
	...
	hips.position.y = .92;
	root.add(hips);
	add(new Mesh(new CylinderGeometry(.13, .16, .38, 10), cloth), hips).position.y = .12;
	const skirt = new Mesh(new ConeGeometry(.42, .95, 12, 1, true), cloth);
	skirt.rotation.x = Math.PI;
	skirt.position.y = -.42;
	hips.add(skirt);
	add(new Mesh(new TorusGeometry(.16, .018, 8, 16), gold), hips).position.y = .28;
	add(new Mesh(new TorusGeometry(.07, .012, 6, 12), gold), hips).position.set(0, .34, -.12);
	const chest = new Group();
	chest.position.y = 1.28;
	root.add(chest);
	add(new Mesh(new SphereGeometry(.16, 12, 10), skin), chest).scale.set(1.05, .85, .7);
	const head = new Group();
	head.position.y = 1.52;
	root.add(head);
	add(new Mesh(new SphereGeometry(.13, 14, 12), skin), head);
	face.colorSpace = SRGBColorSpace;
	face.wrapS = face.wrapT = ClampToEdgeWrapping;
	const faceMat = new MeshBasicMaterial({ map: face });
	const faceMesh = new Mesh(new CircleGeometry(.105, 20), faceMat);
	faceMesh.position.set(0, .01, -.11);
	faceMesh.rotation.y = Math.PI;
	head.add(faceMesh);
	const earL = new Mesh(new ConeGeometry(.03, .1, 6), skin);
	earL.rotation.z = .7;
	earL.position.set(-.13, .02, 0);
	const earR = earL.clone();
	earR.rotation.z = -.7;
	earR.position.x = .13;
	head.add(earL, earR);
	add(new Mesh(new SphereGeometry(.015, 8, 8), glow), head).position.set(-.04, .02, -.12);
	add(new Mesh(new SphereGeometry(.015, 8, 8), glow), head).position.set(.04, .02, -.12);
	const hairCap = new Mesh(new SphereGeometry(.14, 12, 10, 0, Math.PI * 2, 0, Math.PI * .55), hair);
	hairCap.position.y = .04;
	head.add(hairCap);
	for (let i = 0; i < 7; i++) {
		const lock = new Mesh(new CylinderGeometry(.012, .035, .85 - i * .04, 6), hair);
		const a = i / 7 * Math.PI - Math.PI * .5;
		lock.position.set(Math.sin(a) * .08, -.28, .1 + Math.abs(Math.cos(a)) * .04);
		lock.rotation.x = .25;
		lock.rotation.z = a * .15;
		head.add(lock);
	}
	const makeArm = (side) => {
		const g = new Group();
		g.position.set(side * .2, 1.32, 0);
		const upper = new Mesh(new CylinderGeometry(.035, .04, .32, 8), skin);
		upper.position.y = -.16;
		g.add(upper);
		const lower = new Group();
		lower.position.y = -.32;
		const lowM = new Mesh(new CylinderGeometry(.028, .032, .3, 8), skin);
		lowM.position.y = -.14;
		lower.add(lowM);
		g.add(lower);
		root.add(g);
	...
	hand.position.set(0, -.3, -.08);
	right.lower.add(hand);
	const makeLeg = (side) => {
		const g = new Group();
		g.position.set(side * .09, .92, 0);
		const thigh = new Mesh(new CylinderGeometry(.045, .05, .38, 8), skin);
		thigh.position.y = -.18;
		g.add(thigh);
		const shin = new Mesh(new CylinderGeometry(.035, .04, .4, 8), skin);
		shin.position.y = -.52;
		g.add(shin);
		const foot = new Mesh(new BoxGeometry(.07, .04, .14), gold);
		foot.position.set(0, -.74, -.03);
		g.add(foot);
		root.add(g);
		return g;
	};
	...
	mkGun((g) => {
		g.add(mesh$1(new BoxGeometry(.04, .06, .28), dark, 0, 0, -.12));
		const b = new CylinderGeometry(.015, .018, .32, 6);
		b.rotateX(Math.PI / 2);
		g.add(mesh$1(b, dark, 0, .01, -.32));
		g.add(mesh$1(new BoxGeometry(.03, .03, .04), glow, 0, .02, -.48));
	});
	mkGun((g) => {
		const staff = new CylinderGeometry(.012, .016, .42, 6);
		staff.rotateX(Math.PI / 2);
		g.add(mesh$1(staff, dark, 0, 0, -.18));
		g.add(mesh$1(new OctahedronGeometry(.05, 0), glow, 0, .02, -.4));
	});
	mkGun((g) => {
		g.add(mesh$1(new BoxGeometry(.07, .07, .2), dark, 0, 0, -.08));
		const b = new CylinderGeometry(.01, .012, .18, 6);
		b.rotateX(Math.PI / 2);
		g.add(mesh$1(b, glow, -.02, .01, -.22));
		g.add(mesh$1(b.clone(), glow, .02, .01, -.22));
	});
	mkGun((g) => {
		g.add(mesh$1(new TorusGeometry(.055, .01, 6, 12), gold, 0, .04, -.16));
		const beam = new CylinderGeometry(.012, .02, .28, 6);
		beam.rotateX(Math.PI / 2);
		g.add(mesh$1(beam, glow, 0, .02, -.3));
	});
	mkGun((g) => {
		const b = new CylinderGeometry(.01, .014, .55, 6);
		b.rotateX(Math.PI / 2);
		g.add(mesh$1(b, dark, 0, .02, -.28));
		g.add(mesh$1(new BoxGeometry(.04, .03, .1), dark, 0, .06, -.12));
		g.add(mesh$1(new BoxGeometry(.02, .02, .04), glow, 0, .02, -.56));
	});
	mkGun((g) => {
		g.add(mesh$1(new BoxGeometry(.06, .05, .22), dark, 0, 0, -.08));
		const r = new CylinderGeometry(.008, .008, .36, 6);
		r.rotateX(Math.PI / 2);
		g.add(mesh$1(r, glow, -.02, .02, -.28));
		g.add(mesh$1(r.clone(), glow, .02, .02, -.28));
	});
	worldGuns[0].visible = true;
	return {
		root,
		head,
		leftArm: left.g,
...
function mesh$1(geo, mat, x, y, z) {
	const m = new Mesh(geo, mat);
	m.position.set(x, y, z);
	return m;
}
function poseHunter(rig, bob, grounded, speed, pitch, weapon) {
	const swing = grounded ? Math.sin(bob) * Math.min(1, speed / 7) * .45 : .2;
	rig.leftArm.rotation.x = swing;
	rig.rightArm.rotation.x = -swing * .35 - .55;
	rig.leftLeg.rotation.x = -swing * .7;
	rig.rightLeg.rotation.x = swing * .7;
	rig.head.rotation.x = MathUtils.clamp(-pitch * .35, -.4, .35);
	for (let i = 0; i < rig.worldGuns.length; i++) rig.worldGuns[i].visible = i === weapon % rig.worldGuns.length && weapon < 6;
}
var FX = [
	"emberFloor",
	"slowEnemies",
...
var CrimsonGame = class {
	...
	constructor(canvas) {
		...
		this.camera = new PerspectiveCamera(78, 1, .08, 420);
	...
	async start() {
		...
		if (this.disposed) return;
		this.tell("The palace is ahead. Walk the grounds. Claim four Names, then the throne.");
		this.timer.reset();
		this.timer.update();
	...
	async loadWorld() {
		...
		const skyT = loader.load("/textures/sky.jpg");
		skyT.colorSpace = SRGBColorSpace;
		skyT.mapping = 303;
		this.textures.push(floorT, wallT, colT, energyT, waterT, skyT);
		this.mats.floor = new MeshBasicMaterial({
			map: floorT,
			color: new Color(2.15, 1.75, 1.55)
		});
		this.mats.wall = new MeshBasicMaterial({
			map: wallT,
			color: new Color(2.45, 1.95, 1.7)
		});
		this.mats.column = new MeshBasicMaterial({
			map: colT,
			color: new Color(2.25, 1.85, 1.55)
		});
		...
		this.mats.water = new MeshBasicMaterial({
			map: waterT,
			color: 6953e3,
			transparent: true,
			opacity: .88
		...
		this.mats.ember = new MeshStandardMaterial({
			color: this.profile.ember,
			emissive: this.profile.ember,
			emissiveIntensity: 1.2 + this.profile.glow * .4,
			roughness: .35,
			metalness: .2
		});
		this.mats.body = new MeshStandardMaterial({
			color: 1707026,
			roughness: .55,
			metalness: .25,
			emissive: 4851216,
		...
		this.scene.background = new Color(4855836);
		this.scene.fog = new Fog(3805208, 140, 420);
		const hemi = new HemisphereLight(16754824, 1574928, 1.65);
		const dir = new DirectionalLight(16765104, 2.1);
		...
		const sky = new Mesh(new SphereGeometry(380, 24, 16), new MeshBasicMaterial({
			map: skyT,
			side: 1,
			depthWrite: false,
			fog: false
		...
		this.camera.rotation.set(-.16, 0, 0);
	...
	enemyMesh(kind) {
		...
		const ember = this.mats.ember;
		if (kind === "wraith") {
			g.add(mesh(new CapsuleGeometry(.28, 1, 4, 8), body, 0, .9, 0));
			g.add(mesh(new ConeGeometry(.35, 1.1, 6), ember, 0, .4, 0));
		} else if (kind === "sentinel") {
			g.add(mesh(new IcosahedronGeometry(.55, 0), body, 0, 1.2, 0));
			const t = new Mesh(new TorusGeometry(.7, .06, 6, 16), ember);
			t.rotation.x = Math.PI / 2;
			t.position.y = 1.2;
			g.add(t);
		} else if (kind === "construct") {
			g.add(mesh(new BoxGeometry(1.1, 1.4, .7), body, 0, 1.1, 0));
			g.add(mesh(new BoxGeometry(.7, .7, .7), ember, 0, 2, 0));
		} else if (kind === "shade") g.add(mesh(new SphereGeometry(.42, 10, 10), ember, 0, 1, 0));
		else {
			g.add(mesh(new ConeGeometry(1.1, 2.6, 8), this.mats.ember, 0, 1.4, 0));
			g.add(mesh(new CapsuleGeometry(.38, .8, 4, 8), body, 0, 3, 0));
			g.add(mesh(new SphereGeometry(.32, 10, 10), body, 0, 3.7, 0));
			g.add(mesh(new BoxGeometry(1.6, .12, .12), ember, 0, 3.2, 0));
		}
		return g;
	}
	spawnBoss() {
	...
	frame() {
		...
		if (useGame.getState().screen === "playing" && !this.paused && !this.ended) {
			if (this.input.hasActivity() || this.input.padConnected) this.locked = true;
			this.acc += dt;
			const step = 1 / 60;
			while (this.acc >= step) {
				this.fixed(step);
	...
	setQuality(q) {
		this.quality = q;
		const pr = q >= 1080 ? Math.min(2, window.devicePixelRatio) : q >= 720 ? 1 : .7;
		this.renderer.setPixelRatio(pr);
		this.resize();
	}
	look(dt) {
		if (this.overlayOpen() || this.grace > .25) {
	...
	move(dt) {
		...
		const rate = this.grounded ? 14 : 3.2;
		const lerp = 1 - Math.exp(-rate * dt);
		this.vx += (wishX - this.vx) * lerp;
		this.vz += (wishZ - this.vz) * lerp;
		...
		this.slide(dt);
		if (!wasGrounded && this.grounded) this.audio.land(Math.min(1, Math.abs(fall) / 14));
	...
	weaponsTick(dt) {
		...
		const w = WEAPONS[this.weapon];
		if (!w) return;
		if (!this.locked && !this.input.isCoarse && !this.input.padConnected) return;
	...
	shoot() {
		const w = WEAPONS[this.weapon];
		const cd = w.cooldown * (w.fire === "hitscan" ? lerp(1, this.profile.spreadMul, .15) : 1);
		...
		if (w.fire !== "melee") this.mag[this.weapon]--;
		this.audio.fireWeapon(w.view ?? 0, w.fire, !!w.automatic);
		const recoil = w.recoil * this.profile.recoilMul;
		this.recoil = recoil * 9;
		this.trauma = Math.min(1, this.trauma + (useGame.getState().settings.shake ? recoil * 4 : 0));
		this.pitch += recoil;
		this.muzzleT = .05;
		const mods = this.mods();
		const forti = this.fortitudeT > 0 ? 1.35 : 1;
		const crit = this.coilT > 0 || Math.random() < mods.crit ? 1.6 : 1;
		const dmg = w.damage * this.profile.damageMul * mods.dmg * forti * crit;
		if (w.fire === "melee") this.melee(dmg, w.range);
		else if (w.fire === "projectile") this.spawnProj(false, 22, dmg, .22, void 0, void 0, true);
		else if (w.fire === "rail") {
			this.hitscan(dmg, w.spread * this.profile.spreadMul, w.range);
			this.flashLance(w.range * .55);
		} else {
			const spread = w.spread * this.profile.spreadMul;
	...
	hitscan(dmg, spread, range) {
		...
		while (obj) {
			if (obj.userData.eid) {
				this.hurtEnemy(obj.userData.eid, dmg, h.point);
				return;
			}
			obj = obj.parent;
		}
	...
	spawnProj(fromEnemy, speed, dmg, radius, origin, dir, grav = false) {
		...
		if (!p) {
			const m = new Mesh(new SphereGeometry(.12, 8, 8), this.mats.ember);
			p = {
				mesh: m,
				vx: 0,
				vy: 0,
	...
	projectilesTick(dt) {
		for (let i = this.projectiles.length - 1; i >= 0; i--) {
			...
			p.life -= dt;
			let dead = p.life <= 0;
			if (p.grav && p.mesh.position.y < .12) {
				this.blast(p.mesh.position, 3.6, p.dmg * .7);
				dead = true;
			} else if (p.mesh.position.y < 0) dead = true;
			if (!dead) for (const b of this.world.colliders) {
				const q = p.mesh.position;
				if (q.x > b.minX && q.x < b.maxX && q.y > b.minY && q.y < b.maxY && q.z > b.minZ && q.z < b.maxZ) {
					if (p.grav && !p.fromEnemy) this.blast(q, 3.6, p.dmg * .7);
					else this.spark(q, .8);
					dead = true;
					break;
				}
	...
	hurtPlayer(dmg) {
		...
		const taken = dmg * ((this.fortitudeT > 0 ? .55 : 1) * (1 - this.mods().resist));
	...
	enemiesTick(dt) {
		...
		for (const e of this.enemies) {
			...
			if (e.kind === "sentinel") this.enemySentinel(e, dt, dist, dx, dz);
			else if (e.kind === "shade") this.enemyShade(e, dt, dist, dx, dz);
			else if (e.kind === "construct") this.enemyConstruct(e, dt, dist, dx, dz);
			else this.enemyWraith(e, dt, dist, dx, dz);
	...
	visual(dt) {
		const bobY = Math.sin(this.bob) * (this.grounded ? .045 : 0);
		...
		if (this.hunter) {
			...
			poseHunter(this.hunter, this.bob, this.grounded, Math.hypot(this.vx, this.vz), this.pitch, w?.view ?? 0);
		}
		if (fps) this.camera.position.set(0, 0, 0);
		else if (cam === "tps") this.camera.position.set(.45, .32, 3.2);
		else this.camera.position.set(.15, 1.6, 8.2);
		const ads = this.ads();
		...
		const tz = ads ? -.48 : -.58 + this.recoil * .08;
		this.viewmodel.position.x += (tx - this.viewmodel.position.x) * (1 - Math.exp(-12 * dt));
		this.viewmodel.position.y += (ty - this.viewmodel.position.y) * (1 - Math.exp(-12 * dt));
		this.viewmodel.position.z += (tz - this.viewmodel.position.z) * (1 - Math.exp(-12 * dt));
		this.viewmodel.rotation.x = this.recoil * .15;
		...
		for (const e of this.enemies) e.hit.traverse((o) => {
	...
	pushHud() {
		const w = WEAPONS[this.weapon];
		const zone = this.world.getZone(this.px, this.py, this.pz);
	...
	render() {
		...
		if ((useGame.getState().settings.cam || "fps") === "fps") {
